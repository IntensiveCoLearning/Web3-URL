...
- **共学证明打卡：** 你必须每天提交你的学习笔记到这个共学仓库来证明你今天学习了，仓库会自动更新你的打卡状态，**每周**有两次请假的机会，超过后状态变为 ❌，视为本次共学失败。
...
1. 直接 Fork 这个仓库，免费报名🌟。
2. 复制 Template_WICL1st.md文件 （这个非常重要！）创建你的个人笔记，按照文档内提示要求填写你的信息，并且重命名为你的名字：YourName_WICL1st.md。
3. 创建一个 PR 到当前仓库，本残酷共学助教会对你的 PR 进行 review，review 通过后，你的 PR 会被 merge 到 main 分支，这个时候你会收到邀请加入这个仓库 contribution 的邮件，接受邀请后，你会自动获得 main 分支的 push 权限，这个时候表示你报名成功了，后续就可以将你的学习记录直接 push 到本仓库的 main 分支进行更新。
4. 请加入 Telegram 群组保持交流：https://t.me/LXDAO/8748 （LXDAO TG #Web3 URL IntensiveCoLearning 频道）。请加入群组并且在群里报到一下方便助教记录。
- 本次活动的打卡记录会存放在下方👇「**Web3 URL 残酷共学记录表**」。
---
